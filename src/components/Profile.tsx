...
export default function Perfil({ profile }: ProfileProps) {
  ...
  const unlockedAchievements = profile.achievements.filter(
  ...
  const groupedAchievements = Array.from(
    { length: Math.ceil(totalAchievements / achievementsPerPage) },
    (_, i) =>
      unlockedAchievements.slice(
        i * achievementsPerPage,
        (i + 1) * achievementsPerPage
      )
  );

  return (
    <div className="max-w-4xl mx-auto p-6 m-10 bg-white shadow-lg rounded-lg ">
        ...
        <div className="relative mt-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              ...
              disabled={currentIndex === groupedAchievements.length - 1}
              className="bg-gray-300 p-2 rounded-full"
            >
              <BsArrowRight />
            </button>
